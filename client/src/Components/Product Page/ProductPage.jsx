...
const ProductPage = () => {
  ...
  const btncategory = [...new Set(Ppdata.map((data) => data.category))];
      ...
      <div className="flex justify-center  ">
        <div className={showDiv2 ? "max-[900px]:block" : "max-[900px]:hidden"}>
          <div className="bg-white border-border border-solid border-2 h-auto w-60 m-4 mt-32 p-5 rounded max-[900px]:w-auto max-[900px]:h-full max-[900px]:m-0 max-[900px]:translate-x-full ">
            <button
              className="flex justify-end hover:cursor-pointer min-[901px]:hidden  bg-transparent border-none"
              onClick={toggleDivs}
            >
              <FaTimes size={25} className="text-black" />
            </button>
            <p className="font-body text-white bg-primary p-3 rounded">
              Categories
            </p>
            <button
              className="border-border border-solid border-2 rounded bg-white text-ptext container p-3 text-left font-body mb-3 cursor-pointer hover:bg-primary hover:text-white focus:bg-primary focus:text-white"
              onClick={() => {
                setItems(Ppdata);
                proLength();
              }}
            ...
            />
            <button className="border-border border-solid border-2 rounded bg-white text-ptext container p-3 text-left font-body mb-3 cursor-pointer hover:bg-primary hover:text-white active:bg-primary active:text-white  ">
              Stars
            </button>
            <button className="border-border border-solid border-2 rounded bg-white text-ptext container p-3 text-left font-body mb-3 cursor-pointer hover:bg-primary hover:text-white active:bg-primary active:text-white  ">
              Date added
        ...
        <div className={showDiv1 ? "max-[900px]:block" : "max-[900px]:hidden"}>
          <div className="mt-20 mb-10 max-[900px]:mt-5   ">
            <button
              className="ml-5 hover:cursor-pointer min-[901px]:hidden bg-transparent border-none "
              onClick={toggleDivs}
            >
              <VscSettings size={25} className="text-black" />
            </button>
            <div className="flex text-ptext font-body justify-between m-3 -mb-3 ">
              <p className="-ml-48">
                {" "}
                <input
                  type="text"
                  value={length}
                  className="bg-transparent border-none text-primary font-bold font-body text-right pr-1 outline-none text-base"
                />
                Products
              </p>{" "}
              <p>
                ...
                <input
                  type="text"
                  value={value}
                  className="bg-transparent border-none text-primary font-bold font-body text-base -mr-36 pr-0 outline-none"
                />
              </p>
            </div>
                  ...
                  <div className="border-border border-solid border-2 border-t-0 p-2 ">
                    ...
                    <div className="flex">
                      <Link
                        ...
                        to={"/productdetails"}
                      >
                        <button className="text-white font-body bg-black border-0 rounded-full px-3 py-2 mr-1 text-xs hover:opacity-75 cursor-pointer max-[900px]:px-1 max-[900px]:py-1 ">
                          DETAILS
                        </button>
                      </Link>{" "}
                      <Link className="no-underline text-white" to={""}>
                        <button
                          className="text-white bg-primary border-0 rounded-full p-2 px-5 text-xs hover:opacity-75 cursor-pointer font-body max-[900px]:px-1 max-[900px]:py-1 "
                          onClick={() => handleAddToCart(data)}
                        >
                          ADD TO CART{" "}
                          <FaShoppingCart className="max-[900px]:hidden" />{" "}
                        </button>
                      </Link>
                    </div>
                  </div>
